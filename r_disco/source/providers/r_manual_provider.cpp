...
vector<r_stream_config> r_manual_provider::_fetch_configs(const string& top_dir)
{
    ...
    if(r_fs::file_exists(manual_config_path))
    {
        ...
        for(auto device : doc["manual_provider_config"]["devices"])
        {
            ...
            if(_agent && _agent->is_recording(id))
                continue;
            ...
            r_nullable<string> camera_name;
            if(device.contains("camera_name"))
            {
                camera_name.set_value(device["camera_name"]);
            }
            ...
            stream_config.id = id;
            stream_config.camera_name = camera_name;
