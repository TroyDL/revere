...
r_cirrus_device_info_agent::r_cirrus_device_info_agent()
{
}

r_cirrus_device_info_agent::~r_cirrus_device_info_agent() noexcept
{
}

r_nullable<r_device_info> r_cirrus_device_info_agent::get_device_info(const string& ssdpNotifyMessage)
{
    r_nullable<r_device_info> output;

    try
    {
        if(r_string_utils::contains(r_string_utils::to_lower(ssdpNotifyMessage), "cirrus"))
        {
            ...
            if(locStart != string::npos)
            {
                ...
                r_device_info di;
                di.unique_id = ssdpNotifyMessage.substr(uuidStart + 5, 36); //+5 start past "uuid:"
                di.rtsp_url = rtsp;
                di.type = "video";
