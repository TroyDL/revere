...
public:
    ...
    r_fake_camera(const r_fake_camera&) = delete;
    r_fake_camera(r_fake_camera&&) = delete;

    ~r_fake_camera() noexcept;

    r_fake_camera& operator=(const r_fake_camera&) = delete;
    r_fake_camera& operator=(r_fake_camera&&) = delete;

    void start() const;
    void quit() const;

    static std::string make_data_uri(const std::string& type, const uint8_t* p, size_t len);
