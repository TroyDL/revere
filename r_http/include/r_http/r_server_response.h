...
//     "multipart/mixed; boundary="foo"
    ...
    void write_response(r_utils::r_stream_io& socket);

    // Chunked transfer encoding support...
    void write_chunk(r_utils::r_stream_io& socket, size_t sizeChunk, const void* bits);
    void write_chunk_finalizer(r_utils::r_stream_io& socket);

    // Multipart mimetype support
    // WritePart() will automaticaly add a Content-Length header per
    // part.

    void write_part( r_utils::r_stream_io& socket,
                     const std::string& boundary,
                     const std::map<std::string,std::string>& partHeaders,
                     void* chunk,
                     uint32_t size );

    void write_part_finalizer(r_utils::r_stream_io& socket, const std::string& boundary);
