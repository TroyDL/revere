...
void r_client_request::write_request( r_stream_io& socket ) const
{
    ...
    if( (_method == METHOD_POST || _method == METHOD_PATCH || _method == METHOD_PUT) &&
        !r_string::contains(_contentType, "x-www-form-urlencoded") && _body.size() )
    {
...
{
    const std::string auth = r_string::format( "%s:%s", user.c_str(), pass.c_str() );

    _authData = r_string::to_base64( auth.c_str(), auth.length() );
}
...
std::string r_client_request::_get_headers_as_string( r_stream_io& socket ) const
{
    ...
    if( _method == METHOD_POST || _method == METHOD_PATCH || _method == METHOD_PUT )
    {
        if(r_string::contains(_contentType,"x-www-form-urlencoded"))
        {
            ...
            {
                body += r_string::format( "%s=%s&", kv.first.c_str(), kv.second.c_str() );
            }

            body = body.substr( 0, body.length() - 1 );

            msgHeader += r_string::format( "Content-Length: %d\r\n\r\n", body.length() );
            msgHeader += body;
        }
        else
        {
