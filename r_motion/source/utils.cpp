...
r_image r_motion::argb_to_gray8(const r_image& argb)
{
    ...
    vector<uint8_t> result(argb.width * argb.height);
...
{
    const uint8_t* src = argb.data.data();
    vector<uint8_t> result((argb.width*2) * argb.height);
    uint8_t* dst = result.data();
...
r_image r_motion::gray8_to_argb(const r_image& gray)
{
    ...
    vector<uint8_t> result((gray.width*4) * gray.height);
    uint8_t* dst = result.data();
...
{ 
    ...
    vector<uint8_t> result(a.width * a.height);
    uint8_t* dst = result.data();
...
{
    vector<uint8_t> output_buffer(a.width * a.height);

    for(uint16_t h = 0; h < a.height; ++h)
    {
        for(uint16_t w = 0; w < a.width; ++w)
        {
            if(a.data[(h*a.width) + w] > 0)
            {
                if(b.data[(h*b.width) + w] == 0)
                    output_buffer[(h*a.width) + w] = a.data[(h*a.width) + w];
                else
                    output_buffer[(h*a.width) + w] = 0;
            }
        }
    }
...
r_image r_motion::average_images(const std::deque<r_image>& images)
{
    ...
    vector<uint8_t> result((width*src_pixel_size) * height);
    uint8_t* dst = result.data();
    ...
    for(uint16_t h = 0; h < height; ++h)
    {
        for(uint16_t w = 0; w < width; ++w)
        {
            for(int i = 0; i < src_pixel_size; ++i)
            {
                ...
                {
                    sum += image.data[(h*width*src_pixel_size) + (w*src_pixel_size) + i];
                }
