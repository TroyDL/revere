...
static r_connection _parse_connection(const std::string& c)
{
    ...
    cd.connection_address = r_string::strip(connParts[2]);
    return cd;
}
...
static r_bandwidth _parse_bandwidth(const std::string& b)
{
    ...
    r_bandwidth bw;
    bw.type = bwParts[0];
    bw.value = r_string::strip(bwParts[1]);
    return bw;
}
...
static r_encryption _parse_encryption(const std::string& k)
{
    ...
    if(r_string::contains(k, ":"))
    {
        ...
        enc.key = r_string::strip(keyParts[1]);
    }
    else enc.method = k;
    return enc;
}
...
static void _parse_media_description(r_media_description& md, const std::string& m)
{
    ...
    md.proto = mparts[2];
    md.fmt = r_string::strip(mparts[3]);
}
...
r_session_description r_rtsp::parse_sdp(const std::string& sdp)
{
    ...
    for(auto l : globalLines)
    {
        ...
        if(lt == "v=")
            sdesc.protocol_version = r_string::strip(lv);
        else if(lt == "o=")
        {
            ...
            if(numParts>0)
                sdesc.origin.username = r_string::strip(originParts[0]);
            if(numParts>1)
                sdesc.origin.session_id = r_string::strip(originParts[1]);
            if(numParts>2)
                sdesc.origin.session_version = r_string::strip(originParts[2]);
            if(numParts>3)
                sdesc.origin.nettype = r_string::strip(originParts[3]);
            if(numParts>4)
                sdesc.origin.addrtype = r_string::strip(originParts[4]);
            if(numParts>5)
                sdesc.origin.unicast_address = r_string::strip(originParts[5]);
        }
        else if(lt == "s=")
                ...
                if(r_string::contains(lv, ":"))
                {
                    ...
                    if(mdesc.control.empty())
                    {
                        if(r_string::contains(lv, "control"))
                            mdesc.control = r_string::strip(lv.substr(lv.find("control:") + 8));
                    }
                }
...
{
    auto sps = r_string::from_base64(encodedSPS);
    auto pps = r_string::from_base64(encodedPPS);
