...
#define RTF_ASSERT_THROWS(thing_that_throws,what_is_thrown) do { try { bool threw = false; try { thing_that_throws; } catch( what_is_thrown& ex ) { threw=true; } catch(r_exception& ex) { printf("SOMETHING THREW  %s\n",ex.what()); } if(!threw) throw false; } catch(...) { printf("IT THREW!\n"); throw std::runtime_error(rtf_format("Expected exception not thrown at Line:%d File:%s", __LINE__, __FILE__)); } } while(false)

#define RTF_ASSERT_NO_THROW(thing_that_doesnt_throw) do { bool threw = false; std::string msg; try { thing_that_doesnt_throw; } catch(std::exception& ex) { msg = ex.what(); threw=true; } catch( ... ) { threw=true; } if(threw) { throw std::runtime_error(rtf_format("Unexpected exception at Line:%d File:%s msg=(%s)", __LINE__, __FILE__,msg.c_str())); } } while(false)
