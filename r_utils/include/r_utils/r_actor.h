...
class r_actor
{
 public:
    r_actor() = default;
    r_actor(const r_actor&) = delete;
    r_actor(r_actor&&) noexcept = delete;

    virtual ~r_actor() noexcept
    {
        if(started())
            stop();
    }

    r_actor& operator=(const r_actor&) = delete;
    r_actor& operator=(r_actor&&) noexcept = delete;
