...
public:
    ...
    r_byte_ptr_rw(const r_byte_ptr_rw& other) :
        _start(other._start),
    ...
    r_byte_ptr_rw(r_byte_ptr_rw&& other ) noexcept :
    ...
    r_byte_ptr_rw(uint8_t* data, size_t length) :
        _start(data),
        _current(data),
        _length(length)
    {
    ...
    ~r_byte_ptr_rw() noexcept
    {
    }

    r_byte_ptr_rw& operator=(const r_byte_ptr_rw& other)
    {
        return set_ptr(other._start, other._length);
    }

    r_byte_ptr_rw& operator=(r_byte_ptr_rw&& other) noexcept
    {
        _start = std::move(other._start);
        _current = std::move(other._current);
        _length = std::move(other._length);
    ...
    static r_byte_ptr_rw append(std::vector<uint8_t>& vec, size_t space)
    {
    ...
    r_byte_ptr_rw& set_ptr(uint8_t* data, size_t length)
    {
    ...
    void bulk_write( const uint8_t* src, uint32_t len )
    {
    ...
    void bulk_read( uint8_t* dst, uint32_t len )
    {
        if (!_in_bounds(_start, _length, _current, len))
        {
            R_STHROW(r_internal_exception, ("Attempt to read outside the buffer bounds. Start (%p), End (%p), Ptr (%p), Size (%llu)",
    ...
    uint8_t* operator+(size_t step) const
    {
    ...
    bool operator==(const r_byte_ptr_rw& other) const
    {
...
public:
    ...
    /// Copies the current state of the other pointer
    r_byte_ptr_ro(const r_byte_ptr_ro& other) :
        _start(other._start),
        _current(other._current),
        _length(other._length)
    {
    ...
    r_byte_ptr_ro(r_byte_ptr_ro&& other ) noexcept :
        _start(std::move(other._start)),
        _current(std::move(other._current)),
        _length(std::move(other._length))
    {
    ...
    /// Initializes the pointer to point to the beginning of the specified buffer
    r_byte_ptr_ro(const uint8_t* data, size_t length) :
        _start(data),
        _current(data),
        _length(length)
    {
    ...
    r_byte_ptr_ro(const std::pair<size_t, const uint8_t*>& region) :
    ...
    ~r_byte_ptr_ro() noexcept
    {
    }

    r_byte_ptr_ro& operator=(const r_byte_ptr_ro& other)
    {
        return set_ptr(other._start, other._length);
    }

    r_byte_ptr_ro& operator=(r_byte_ptr_ro&& other) noexcept
    {
        _start = std::move(other._start);
        _current = std::move(other._current);
        _length = std::move(other._length);
    ...
    /// Resets the pointer to point to the beginning of the specified buffer
    r_byte_ptr_ro& set_ptr(const uint8_t* data, size_t length)
    {
        _start = _current = data;
        _length = length;
        return *this;
    ...
    /// The original pointer value set on the object (i.e. the beginning of the buffer).
    /// @note This value is not affected by operations on the pointer value.
    const uint8_t* original_ptr() const
    {
        return _start;
    }

    /// Adjusts the current pointer position to point to the specified position in the buffer.
    r_byte_ptr_ro& seek(size_t offset)
    {
        if (!_in_bounds(_start, _length, _start + offset, 1))
	        R_STHROW(r_invalid_argument_exception, ("Attempt to seek outside the buffer bounds. Offset (%llu), length(%llu)", offset, _length));
        _current = _start + offset;
    ...
    const uint8_t* operator+(size_t step) const
    {
        return _current + step;
    }
    ...
    const uint8_t* operator+=(size_t step)
    {
        return _current += step;
    }
    ...
    bool operator==(const r_byte_ptr_ro& other) const
    {
        return other._current == _current;
    }
