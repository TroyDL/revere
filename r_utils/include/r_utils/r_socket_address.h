...
{
public:
    r_socket_address(int port, const std::string& address = ip4_addr_any);

    r_socket_address(const struct sockaddr* addr, const socklen_t len);

    r_socket_address(r_socket_address&& obj) noexcept;

    r_socket_address(const r_socket_address& obj);

    ~r_socket_address() noexcept;

    r_socket_address& operator = (r_socket_address&& obj) noexcept;

    r_socket_address& operator = (const r_socket_address& obj);
    ...
    bool operator==(const r_socket_address&) const;
