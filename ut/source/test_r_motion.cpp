...
static vector<uint8_t> decode_h264_frame(AVCodecID codec_id, AVPixelFormat fmt, uint16_t w, uint16_t h, const uint8_t* data, size_t size)
{
...
void test_r_motion::test_basic_utils()
{
    ...
    if(!fakey_root.empty())
    {
        auto path = fakey_root + r_utils::r_fs::PATH_SLASH + "conv.mp4";
